...
@JsonMap({})
class ContainerGroupNetworkingConfiguration(BaseModel):
    """Network configuration for container groups that defines connectivity, routing, and access control settings

    :param auth: Whether authentication is required for network access to the container group
    :type auth: bool
    ...
    :param dns: Domain name or URL endpoint for the container group's network interface
    :type dns: str
    :param load_balancer: The container group networking load balancer.
    :type load_balancer: TheContainerGroupNetworkingLoadBalancer
    ...
    def __init__(
        ...
        auth: bool,
        dns: str,
        load_balancer: TheContainerGroupNetworkingLoadBalancer,
        port: int,
