    ...
    def __init__(
        ...
        **kwargs
        ...
        if available_gpu_batch is not SENTINEL:
            self.available_gpu_batch = self._define_number(
                "available_gpu_batch", available_gpu_batch
            )
        if available_gpu_high is not SENTINEL:
            self.available_gpu_high = self._define_number(
                "available_gpu_high", available_gpu_high
            )
        if available_gpu_low is not SENTINEL:
            self.available_gpu_low = self._define_number(
                "available_gpu_low", available_gpu_low
            )
        if available_gpu_medium is not SENTINEL:
            self.available_gpu_medium = self._define_number(
                "available_gpu_medium", available_gpu_medium
            )
        if on_call_gpu is not SENTINEL:
            self.on_call_gpu = self._define_number("on_call_gpu", on_call_gpu)
        self._kwargs = kwargs
